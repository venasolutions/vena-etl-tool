...
public class ETLToolClearSlicesTest extends ETLToolTest {
	...
	public void testFileToVenaTableWithClearSlicesByColumns() throws UnsupportedEncodingException {
		ETLClient etlClient = mockETLClient();
		String[] args = buildCommand(new String[] {"--jobName", "Loading intersections file", "--venaTable", "--file", "intersectionsFile.csv;format=CSV;clearSlicesByColumns=col1,col2;table=tableName"});
		...
		assertEquals(new ArrayList<String>(Arrays.asList("col1","col2")),((ETLFileToVenaTableStepDTO)step).getClearSlicesColumns());
	}
}
